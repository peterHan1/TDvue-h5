		...
		<div class="lottery-box">
			...
			<div id="lottery1" class="lotterys" v-show="isShow" ref="mybox1">
			    <table border="0" cellpadding="0" cellspacing="0">
			        ...
			        <tr>
			            <td class="lottery-unit lottery-unit-7"><img src="../../image/lottery/3per.png" class="sapce spa"><div class="mask spa"></div></td>
...
<script type="text/ecmascript-6">
	...
	export default {
		...
		data () {
			return {
				apiUrl: 'api/router/op/getInverstResult',
				...
				if (/(iPhone|iPad|iPod|iOS)/i.test(navigator.userAgent)) {
					vm.setupWebViewJavascriptBridge(function (bridge) {
						bridge.callHandler('h5ToNative_ShowRightNavItem', {
							'rightTitle': '我的奖品',
							'url': vm.global.serverPath + 'prize'
						}, function (response) {
						})
					})
				...
				if (lottery.click) {
					...
					if (this.isLogin === false) {
						...
						if (this.sumCount < 100) {
							...
							lottery.getLottery()
							let vm = this
							vm.$http({url: vm.apiUrl, method: 'post', params: {inverstType: 2, score: 100}, headers: {accessId: vm.accessId, accessKey: vm.accessKey}, emulateJSON: true})
								.then((response) => {
									// alert(response.body.content.position)
									let res = response.body.content.position
									if (res === 1) {
										vm.result = 0
										...
										vm.result = 7
										vm.imgUrl = require('../../image/lottery/min3per.png')
										vm.prizeFont = '3%加息券'
										vm.ssShow = true
									}
								})
				...
				if (lottery.click) {
					...
					if (this.isLogin === false) {
						...
						if (this.sumCount < 10) {
							...
							lottery.getLottery()
							let vm = this
							vm.$http({url: vm.apiUrl, method: 'post', params: {inverstType: 1, score: 10}, headers: {accessId: vm.accessId, accessKey: vm.accessKey}, emulateJSON: true})
								.then((response) => {
									let res = response.body.content.position
									if (res === 8) {
										vm.result = 0
